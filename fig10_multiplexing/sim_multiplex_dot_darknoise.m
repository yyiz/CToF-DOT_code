...
savefname = "darknoise_psnr_vs_compression_multiplexing";
...
poissnoise = poissrnd(dcr * (intTime/1000), size(mfull_coloc)) - (dcr * (intTime/1000))* ones(size(mfull_coloc));

mfull_noisy = mfull_coloc .* scaleFac;
mfull_noisy = mfull_noisy + poissnoise; 
...
for i = 1:arrDim
    ...
    max_sig_full = max(sum(bkgfull, 5), [], 'all');
    
    scaleFac = maxPhots ./ max_sig_full;

    Hm_pos = (Hi_pos .* mfull) * scaleFac;
    Hm_neg = (Hi_neg .* mfull) * scaleFac;
    
    noise_pos = poissrnd(dcr * (intTime/1000), size(Hm_pos)) - (dcr * (intTime/1000) * ones(size(Hm_pos)));
    noise_neg = poissrnd(dcr * (intTime/1000), size(Hm_neg)) - (dcr * (intTime/1000) * ones(size(Hm_neg)));

    Hm_pos = Hm_pos + noise_pos; 
    Hm_neg = Hm_neg + noise_neg; 
