function [X_est, errPlot] = fista(b,f,fT,step,sizeX,opts,lam1)
%% Author: Yongyi Zhao, adapting code by Vivek Boominathan
% Vivek Boominathan
% Rice University
% vivekb@rice.edu
...
while 1
    ...
    X_nxt = shrinkOp(X_nxt, step.*lam1);
    if (opts.nonneg)
        X_nxt = max(X_nxt, 0);
    end
...
szX = [size(A,2),1]; %A.szInp; %(1), A.szInp(2), A.szInp(3)];
